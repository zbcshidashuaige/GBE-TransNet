...
class TTAFrame():
    ...
    def test_one_img_from_path_2(self, path):
        ...
        maska  = self.net.forward(img5)[0].squeeze().cpu().data.numpy()  # .squeeze(1)
        maskb  = self.net.forward(img6)[0].squeeze().cpu().data.numpy()
    ...
    def test_one_img_from_path_1(self, img):
        ...
        mask = self.net.forward(img5)[0].squeeze().cpu().data.numpy()  # .squeeze(1)
...
if __name__ == "__main__":
    ...
    test_path = './dataset/whubuild/test/images'
    save_path = './dataset/whubuild/test/save/'
    imgs = os.listdir(test_path)

    model_path = './weights/ResNet50v2_whu_train_loss_best.th'
    ...
    for img in tqdm(imgs, ncols=20, total=len(imgs)):
        ...
        #print(im)#none
        #cv2.imshow("im", im)
        pre = solver.test_one_img_from_path_2(im)
        pre[pre >= 4.0] = 255
        pre[pre < 4.0] = 0
