...
class MyFrame():
    def __init__(self, net, loss1, loss2, lr=2e-4, evalmode=False):
        # self.net = net().cuda()
        self.net = net.cuda()
    ...
    def test_one_img(self, img):
        pred,out1 = self.net.forward(img)
    ...
    def optimize(self):
        ...
        pred,out1 = self.net.forward(self.img)
        loss1 = self.loss1(self.mask, pred)
        loss2,loss3 = self.loss2(out1,self.mask)
        loss = 5 * loss2  + loss1 
        loss.backward()
        self.optimizer.step()
        # return loss.data[0]
